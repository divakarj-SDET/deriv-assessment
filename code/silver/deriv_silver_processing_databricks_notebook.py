...
# MAGIC %md
# MAGIC # Deriv Assessment — Silver Processing
# MAGIC
# MAGIC Production-oriented Silver layer implementation for:
# MAGIC - Batch JSON -> Silver
# MAGIC - Vendor deposit stream -> Silver using `foreachBatch`
# MAGIC - Vendor schema drift (`payment_method` -> `method`)
# MAGIC - Data-quality quarantine
# MAGIC - Client profile CDC -> SCD Type 2
# MAGIC
# MAGIC **Important:** Bronze technical fields such as `_corrupt_record` and
# MAGIC `_rescued_data` are not blindly propagated into Silver. Delta MERGE mappings
# MAGIC are explicit to prevent target/source schema mismatch.

# COMMAND ----------
# COMMAND ----------

# MAGIC ## 2. Reusable Helpers
...
# MAGIC ## 4. Batch Profile → Silver Current
...
# MAGIC ## 7. Streaming Vendor Deposits → Silver — Fixed MERGE
...
# MAGIC ## 8. Client Profile CDC → SCD2
...
# MAGIC ## 10. Streaming Vendor Deposits Execution
...
# MAGIC ## 11. Streaming CDC Execution
...
# MAGIC %md
# MAGIC ## Why the previous version failed
# MAGIC
# MAGIC The streaming Bronze table contains technical columns including
# MAGIC `_source_file_modification_time` and `_corrupt_record`. The failed MERGE
# MAGIC attempted to resolve `_source_file_modification_time` against the Silver
# MAGIC target, where that column was absent.
# MAGIC
# MAGIC This notebook uses explicit `whenMatchedUpdate` and
# MAGIC `whenNotMatchedInsert` mappings. The optional
# MAGIC `_source_file_modification_time` mapping is added only when the target
# MAGIC table actually contains that column.
# MAGIC
# MAGIC **Do not use** `whenMatchedUpdateAll()` or `whenNotMatchedInsertAll()` for
# MAGIC this canonical Silver MERGE.
